...
{
    assert(tp != NULL);
    if (tp->enter_sleep == NULL) return ESP_FAIL;
...
{
    assert(tp != NULL);
    if (tp->exit_sleep == NULL) return ESP_FAIL;
...
{
    bool touched = false;
    assert(tp != NULL && x != NULL && y != NULL && tp->get_xy != NULL);
